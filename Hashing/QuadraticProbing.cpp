...
class QuadraticProbing : public Hash{
    public:
        ...
        //EVC constructor
        //set the type to "Quadratic Probing"
        ...
        //Use quadratic probing strategy to insert an item into the hashtable
        ...
        void insertItem(std::vector<std::string> & vec, std::string str){
            if(vec[hashFunction(str)] == "" || vec[hashFunction(str)] == "DELETED") //check for collisions 
                ...
                {
                    int tempy = pow(i,2);
                    int temp = (hashFunction(str) + tempy) % getTableSize();
        ...
        void removeItem(std::vector<std::string> & vec, std::string str){
            ...
            {
                int i = 1;
                while(i < getTableSize()) //while we haven't quadratically traversed through the entire vector
                {
                    int tempy = pow(i,2);
                    int temp = (hashFunction(str) + tempy) % getTableSize(); //creating the new key and adding i, which increments by 1 through each loop
        ...
        //  and perform quadratic traversal until you find the item or reach
        //  empty space.
        ...
        void rehash(std::vector<std::string> & vec){
                ...
                {
                    if(temp[j] != "")
                        insertItem(vec,temp[j]); //insert the item into the vector using quadratic probing
                }
        }
